...
for sheet in sheets:
    ...
    for _, row in df.iterrows():
        ...
        # GeoJSON location 필드 추가
        if pd.notnull(document["longitude"]) and pd.notnull(document["latitude"]):
            document["location"] = {
                "type": "Point",
                "coordinates": [document["longitude"], document["latitude"]]
            }

        # "tradeTypeCode"에 따른 컬렉션 구분하여 저장
        if document["tradeTypeCode"] == "B1":
            collection_one_room = db["OneRoom"]
            collection_one_room.insert_one(document)
        elif document["tradeTypeCode"] == "B2":
            collection_two_room = db["TwoRoom"]
            collection_two_room.insert_one(document)
